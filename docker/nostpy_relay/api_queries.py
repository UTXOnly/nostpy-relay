...
class ApiQuery:
    async def insert_kind_list(self, conn, cur, kind, allowed, reason, logger):
        try:
            ...
            conn.commit()
            logger.info(
                f"Kind {kind} has been updated with allowed={allowed} and reason='{reason}'"
            )
            return True
    ...
    async def query_kind_list(self, conn, cur, kind, logger):
        try:
            ...
            if result:
                ...
                logger.info(
                    f"Found kind {kind_value} with allowed={allowed} and reason='{reason}'"
                )
                return {"kind": kind_value, "allowed": allowed, "reason": reason}
            else:
    ...
    async def insert_pubkey_list(self, conn, cur, pubkey, allowed, reason, logger):
        try:
            ...
            await conn.commit()
            logger.info(
                f"Pubkey {pubkey} has been updated with allowed={allowed} and reason='{reason}'"
            )
    ...
    async def query_pubkey_list(self, conn, cur, pubkey, logger):
        try:
            await cur.execute(
                """
            ...
            if result:
                ...
                logger.info(
                    f"Found pubkey {pubkey_value} with allowed={allowed} and reason='{reason}'"
                )
                return {"pubkey": pubkey_value, "allowed": allowed, "reason": reason}
            else:
    ...
    async def insert_ip_list(self, conn, cur, ip_address, allowed, reason, logger):
        try:
            await cur.execute(
                """
            ...
            await conn.commit()
            logger.info(
                f"IP address {ip_address} has been updated with allowed={allowed} and reason='{reason}'"
            )
    ...
    async def query_ip_list(self, conn, cur, ip_address, logger):
        try:
            await cur.execute(
                """
            ...
            if result:
                ...
                logger.info(
                    f"Found IP address {ip_value} with allowed={allowed} and reason='{reason}'"
                )
                return {"ip_address": ip_value, "allowed": allowed, "reason": reason}
            else:
